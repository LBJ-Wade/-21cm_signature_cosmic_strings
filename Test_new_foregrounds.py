...
def foreground(l, fg_type):
    ...
    if l[1].ndim == 0:
       ...
       for i in range(0, len(dummy)):
           if l[i] < 1:
               dummy[i] =A * (1100. / (1)) ** beta * (130. ** 2 / 1420. ** 2) ** alpha * (1 + z) ** (2 * alpha)
           else:
               dummy[i] = A * (1100. / (l[i] + 1)) ** beta * (130 ** 2 / 1420 ** 2) ** alpha * (1 + z) ** (
...
for k in range(0,n):
    ...
    grf_norm_fg, std_fg = fg_normalize(grf_fg, foreground_type)



    bins = 300
    epsilon_fgr = 1#e-2
    data_ft =  grf_norm_fg*1e3*-delta_z*epsilon_fgr# +signal_ft(patch_size, wake_size_angle,  angle_per_pixel, shift_wake_angle, False)
    ...
    chi = binned_ps/(foreground(360 * k_bin_cents/ (2 * math.pi), foreground_type)*std_fg**2*bins*delta_z**2*epsilon_fgr**2)
